...
public final class AesGcmFieldCipher implements FieldCipher {
  ...
  public AesGcmFieldCipher(boolean enabled, String base64Key) {
    ...
    if (enabled && !hasKey) {
      throw new IllegalStateException(
          "gatewise.encryption.enabled=true requires gatewise.encryption.key (base64 of 32 bytes)");
    }
  ...
  private static SecretKeySpec parseKey(String base64Key) {
    ...
    try {
      ...
      throw new IllegalStateException("gatewise.encryption.key must be valid base64", e);
    }
    if (raw.length != KEY_BYTES) {
      throw new IllegalStateException(
          "gatewise.encryption.key must decode to 32 bytes for AES-256; got " + raw.length);
    }
