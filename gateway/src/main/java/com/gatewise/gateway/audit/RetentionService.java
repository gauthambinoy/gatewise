...
import com.gatewise.gateway.config.ComplianceProperties;
 ...
 *
 * <p>Runs once a day when {@code gatewise.compliance.retention-delete-enabled} is on (off by
 * default). Deletion is a hard delete: it removes the oldest end of each tenant's hash chain, and
 * the chain stays verifiable from its earliest surviving entry (see {@link
 * AuditService#firstBrokenLink}).
 */
@Service
public class RetentionService {
  ...
  @Scheduled(cron = "${gatewise.compliance.retention-cron:0 0 3 * * *}")
