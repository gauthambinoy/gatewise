package com.gatewise.gateway.demo;
...
import com.gatewise.gateway.config.DemoProperties;
 ...
 * Only active when {@code gatewise.demo.enabled} is true; idempotent (the org is created once, and
 * the sample audit trail only when the tenant has none).
 */
@Component
@ConditionalOnProperty(name = "gatewise.demo.enabled", havingValue = "true")
public class DemoSeeder implements ApplicationRunner {
  ...
  private UUID ensureOrg() {
    ...
    members.save(new Member(id, "maya@demo.gatewise.io", "Maya Rodriguez", "owner", "active"));
    members.save(
        new Member(id, "james@demo.gatewise.io", "James Cole", "security_admin", "active"));
    members.save(new Member(id, "aisha@demo.gatewise.io", "Aisha Patel", "auditor", "active"));
  ...
  private void seedActivity(UUID tenantId) {
    String[] actors = {
      "maya@demo.gatewise.io", "james@demo.gatewise.io", "aisha@demo.gatewise.io", "api-service"
    };
    String[] models = {
      "openai/gpt-4o",
      "openai/gpt-4o-mini",
