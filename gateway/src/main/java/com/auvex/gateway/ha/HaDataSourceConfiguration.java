...
import com.auvex.gateway.config.HaProperties;
...
/**
 * Wires read/write datasource splitting, and only when {@code auvex.ha.replica.enabled=true}.
 *
...
@ConditionalOnProperty(prefix = "auvex.ha.replica", name = "enabled", havingValue = "true")
public class HaDataSourceConfiguration {
  ...
  /**
   * The read-replica datasource, built from {@code auvex.ha.replica.*}.
   *
  ...
  public DataSource replicaDataSource(HaProperties properties) {
    ...
    if (replica.url() == null || replica.url().isBlank()) {
      throw new IllegalStateException(
          "auvex.ha.replica.enabled is true but auvex.ha.replica.url is not set");
    }
