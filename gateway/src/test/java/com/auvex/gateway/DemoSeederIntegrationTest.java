...
class DemoSeederIntegrationTest extends AbstractPostgresIntegrationTest {
  ...
  static void properties(DynamicPropertyRegistry registry) {
    registry.add("auvex.demo.enabled", () -> true);
  }
  ...
  void demoKeyResolvesToASeededTenant() throws Exception {
    mvc.perform(get("/v1/me").header("Authorization", "Bearer auvex_demo_key"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.name").value("Demo Org"));

    mvc.perform(get("/v1/policies").header("Authorization", "Bearer auvex_demo_key"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.length()").value(3));

    mvc.perform(get("/v1/members").header("Authorization", "Bearer auvex_demo_key"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.length()").value(3));
  }
}
