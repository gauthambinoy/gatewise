...
class OidcLoginIntegrationTest extends AbstractPostgresIntegrationTest {
  ...
  static void provider(DynamicPropertyRegistry registry) {
    registry.add("gatewise.auth.sso.test.issuer", () -> "https://issuer.test");
    registry.add("gatewise.auth.sso.test.client-id", () -> "client-abc");
    registry.add("gatewise.auth.sso.test.client-secret", () -> "shh");
    registry.add("gatewise.auth.sso.test.authorization-uri", () -> "https://issuer.test/authorize");
    registry.add("gatewise.auth.sso.test.token-uri", () -> "https://issuer.test/token");
    registry.add("gatewise.auth.sso.test.jwks-uri", () -> "https://issuer.test/jwks");
    registry.add("gatewise.auth.sso.test.redirect-uri", () -> "https://gw/auth/oidc/test/callback");
    registry.add("gatewise.auth.sso.test.tenant-slug", () -> "acme");
  }
