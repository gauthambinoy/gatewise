...
Deploys the Auvex AI governance gateway (Spring Boot) and its React admin console
...
- Fetch subchart dependencies once before installing:
  ...
  ```
  helm dependency update deploy/helm/auvex
  ```
...
helm install auvex deploy/helm/auvex \
  --namespace auvex --create-namespace \
  --set ingress.hosts[0].host=auvex.your-domain.com \
  --set ingress.tls[0].hosts[0]=auvex.your-domain.com \
...
> chart Secret (`--set postgresql.auth.existingSecret=auvex`
...
helm install auvex deploy/helm/auvex \
  ...
  --set externalDatabase.jdbcUrl="jdbc:postgresql://my-rds.example.com:5432/auvex" \
  --set externalDatabase.username=auvex \
  --set externalRedis.host=my-elasticache.example.com \
  --set externalRedis.port=6379 \
  --set ingress.hosts[0].host=auvex.your-domain.com \
  --set ingress.tls[0].hosts[0]=auvex.your-domain.com \
  --set secrets.existingSecret=auvex-prod-secrets
```
...
| `secrets.sessionSecret`      | `AUVEX_SESSION_SECRET`   | Console session signing (`openssl rand -hex 32`) |
...
|-----|---------|-------------|
| `gateway.image.repository` | `ghcr.io/gauthambinoy/auvex-gateway` | Gateway image |
...
| `console.image.repository` | `ghcr.io/gauthambinoy/auvex-console` | Console image |
...
| `ingress.hosts[0].host` | `auvex.example.com` | **Replace** |
| `ingress.hosts[0].paths[].service` | `console` | `console` or `gateway` to choose the backend |
| `ingress.tls` | `auvex-tls` / `auvex.example.com` | cert-manager-issued |
...
```
helm uninstall auvex --namespace auvex
```
