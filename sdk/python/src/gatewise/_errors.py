"""Typed exceptions raised by the Auvex client.
...
so the human-readable reason is always available on ``AuvexError.message`` and the raw
envelope on ``AuvexError.body``.
...
class APIConnectionError(AuvexError):
...
def error_from_response(status_code: int, body: Any) -> AuvexError:
    ...
    Returns:
        An ``AuvexError`` subclass instance, populated from the envelope when available.
    """
    ...
    if cls is None:
        ...
        cls = BadRequestError if 400 <= status_code < 500 else AuvexError
